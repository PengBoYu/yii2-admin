...
```php
use pengboyu\admin\components\MenuHelper;
...
Return of `pengboyu\admin\components\MenuHelper::getAssignedMenu()` has default format like:
...
return format of `pengboyu\admin\components\MenuHelper::getAssignedMenu()` by provide a callback to this method.
...
Using Sparated Menu
-------------------
Second parameter of `pengboyu\admin\components\MenuHelper::getAssignedMenu()` used to get menu on it's own hierarchy.
...
user pengboyu\admin\components\Helper;
